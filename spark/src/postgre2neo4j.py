...
partitionNum = 18
url = 'jdbc:postgresql://xxxxxxxxxxx:5432/InsightDB'
df_node = sqlContext.read.format("jdbc").options(
	url=url,
	driver = "org.postgresql.Driver",
	dbtable = 'TECH_NODE',
	user = "xxxxxxxxxxx",
	password = "xxxxxxxxxxx").option('numPartitions',partitionNum).option('lowerBound',1).option('upperBound',20).option('partitionColumn',3).load()
...
def writeNode(p):
	# connect to neo4j
	uri = "bolt://xxxxxxxxxxx:7687"
	driver = GraphDatabase.driver(uri,auth=("neo4j","xxxxxxxxxxx"))
...
df_MAX = sqlContext.read.format("jdbc").options(
	...
	user = "xxxxxxxxxxx",
	password = "xxxxxxxxxxx").option('numPartitions',partitionNum).option('lowerBound',1).option('upperBound',20).option('partitionColumn',4).load()
